...
<head>
    ...
    <title>Cetak Peminjaman Barang - {{ $tahun }}</title>
    <style>
        @page {
            size: A4;
            margin: 1.5cm;
...
<body onload="window.print()">
    ...
    <div class="title-container">
        <div class="title-report">KARTU INVENTARIS BARANG (LAPORAN PEMINJAMAN) - TAHUN {{ $tahun }}</div>
    </div>
        ...
        <tbody>
            @forelse($data as $index => $item)
                <tr>
                    ...
                    <td style="text-align: left; padding-left: 5px;">{{ $item->guru->nama ?? '-' }}</td>
                    <td style="text-align: left; padding-left: 5px;">{{ $item->kode_barang }} (KIB {{ strtoupper($item->jenis_kib) }})</td>
                    <td>{{ $item->jam_pemakaian }}</td>
            ...
            @empty
                <tr>
                    <td colspan="9">Tidak ada data untuk tahun {{ $tahun }}</td>
                </tr>
            @endforelse
        </tbody>
    </table>
